...
class MASK_injection():
	...
	def __init__(self,  initial_min_MASK_injection_mean, initial_min_MASK_injection_mean,
						initial_max_MASK_injection_mean, final_max_MASK_injection_mean, 
	...
	def calc_mean_MASK(self, epoch):
		...
		# for easy logging
		self.mean_MASK_injection = mean_MASK
		self.stdev_MASK_injection = stdev_MASK
	...
	def inject_MASK(self, prediction, key_padding_mask, MASK_injection_mean, MASK_injection_stdev, min_pct=0.01):
		...
		no_MASK = ~torch.any((~MASK_mask) | key_padding_mask, dim=1) # batch x N --> batch,
		...
		MASK_tokens = MASK_tokens[:, :, 20] = 1

		# inject the MASK token
		prediction = torch.where(MASK_mask, MASK_tokens, prediction)
	...
	def MASK_tokens(self, batch):
		...
		batch.predictions, batch.onehot_mask = self.inject_MASK(batch.predictions, batch.key_padding_mask, self.calc_mean_MASK(batch.epoch), self.MASK_injection_stdev)
