...
__device__ float superpose_d_aa(float local_d_aa, int16_t aa_idx, int lane_id) {

	// initialize aa specific accumulators
	float global_d_aa = 0.0f;
	...
	for (int tid = 0; tid < warpSize; ++tid){
		global_d_aa += (lane_id == __shfl_sync(FULL_MASK, aa_idx, tid)) * __shfl_sync(FULL_MASK, local_d_aa, tid); 
	}
...
__global__ void wf_embedding_kernel(
    ...
    float* d_aa_ptr, int stride_d_aa_Z, int stride_d_aa_N, int stride_d_aa_D, int stride_d_aa_A,
	...
	int smem_float_bytes = sizeof(float)*(3 + num_wn + d_model*(1+tot_AA+1));
	...
	for (int thread_mem_idx = thread_id; thread_mem_idx < smem_bytes; thread_mem_idx += num_threads){
	...
	float* out = wavenumbers + num_wn;
	float* d_aa = out + d_model;
	float* d_k = d_aa + num_aa*2;

	// now the half arrays
	__half* aa_magnitudes = reinterpret_cast<__half*>(smem + smem_float_bytes); 
	...
	bool aa_accumulator = lane_id < tot_AA;
	...
	for (int j = 0; j < NJ_iters; ++j){
		...
		for (int k = 0; k < num_wn; ++k) {
			...
			if (aa_accumulator) {
				aa_magnitude = __half2float(aa_magnitudes[k*tot_AA + lane_id]); // swizzling hurt performance more than bank conflicts
			}
			...
			float real_d_aa_superposition = superpose_d_aa(real_d_aa, aa_labels_NJ, lane_id);
			float imag_d_aa_superposition = superpose_d_aa(imag_d_aa, aa_labels_NJ, lane_id);

			if (aa_accumulator) { // have the threads assigned as AA accumulators write to corresponding AA
				atomicAdd(&d_aa[2*k*tot_AA + lane_id], real_d_aa_superposition);
				atomicAdd(&d_aa[(2*k + 1)*tot_AA + lane_id], imag_d_aa_superposition);
			}
		}
	}
	...
	d_aa_ptr += (offs_Z*stride_d_aa_Z) + (offs_NI*stride_d_aa_N);
...
void wf_embedding_kernel_forward(
	...
	int NI_mem = block_size.y * (sizeof(float)*3 + sizeof(float)*(d_model + tot_AA*d_model)); // 3 coordsA, dmodel size output, and tot_AAxd_model for grad wrt to AAs
	int AA_mem = sizeof(__half) * (tot_AA*d_model/2); //  tot_AAs per wavenumber
	int WN_mem = sizeof(float) * d_model/2 * 3; // d_model//2 + d_model = d_model//2 * 3 
