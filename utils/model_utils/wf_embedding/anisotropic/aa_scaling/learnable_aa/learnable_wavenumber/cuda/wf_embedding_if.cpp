...
void wf_embedding_forward(
    ...
    torch::Tensor out, torch::Tensor d_aa, torch::Tensor d_k
) {
    ...
    TORCH_CHECK(d_aa.size(3) == tot_AA, "d_aa d_model size mismatch");

    TORCH_CHECK(d_k.size(0) == tot_Z, "d_aa batch size mismatch");
    TORCH_CHECK(d_k.size(1) == tot_N, "d_aa sequence size mismatch");
    TORCH_CHECK(d_k.size(2) == d_model, "d_aa d_model size mismatch");
