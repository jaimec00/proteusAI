...
class WaveFunctionDecoding(nn.Module):
	def __init__(self,  d_model=512, d_latent=512,
		...
		self.wf_proj = nn.Linear(d_latent, d_model)
		init_xavier(self.wf_proj)
	...
	def forward(self, wf_latent, coords_alpha, key_padding_mask=None): # forward generates the mean and stds, so can use them for loss, 
		...
		for encoder in self.encoders:
			wf_latent = encoder(wf_latent, coords_alpha, key_padding_mask)

		if self.mlp_post is not None:
			wf_latent = wf_latent + self.dropout(self.mlp_post(wf_latent))
		if self.norm_post is not None:
			wf_latent = self.norm_post(wf_latent)

		wf = self.wf_proj(wf_latent)
