...
void attn_fwd(
    ...
    const int* rng_seed_ptr, int stride_rng_seed_Z, int stride_rng_seed_H,
...
void _attn_fwd(
    ...
    TORCH_CHECK(rng_seed.dtype() == torch::kInt32, "rng_seed must be a CUDA tensor");
    TORCH_CHECK(L.dtype() == torch::kFloat32, "L must be a CUDA tensor");
    TORCH_CHECK(O.dtype() == torch::kFloat32, "O must be a CUDA tensor");
    ...
    int nheads*d_k; // feature size (num_wn == d_model//2)
    ...
    float* O_ptr = reinterpret_cast<float*>(O.data_ptr<float>());
