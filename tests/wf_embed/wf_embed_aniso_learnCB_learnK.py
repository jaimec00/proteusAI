...
from utils.model_utils.wf_embedding.anisotropic.cb_scaling.learnable_wavenumbers.cuda.wf_embedding import wf_embedding 
...
def main():
	...
	# assign cb labels and magnitudes, one for each cb, for each wavenumber
	cb_magnitudes = torch.rand((num_wn,), dtype=torch.float32, device=device, requires_grad=True)
	...
	params = [coordsA, coordsB, cb_magnitudes, wavenumbers, mask]
	...
	cuda_d_cb = cb_magnitudes.grad.clone()
	cuda_d_k = wavenumbers.grad.clone()
	cb_magnitudes.grad.zero_()
	wavenumbers.grad.zero_()

	# run torch bwd
	torch_time, torch_mem = profile_bwd(torch_out.sum(), start_event, end_event)
	torch_d_cb = cb_magnitudes.grad.clone()
	torch_d_k = wavenumbers.grad.clone()

	# summarize bwd stats
	rel_error, abs_error = calculate_error(torch_d_cb, cuda_d_cb)
	print(f"cuda d_cb is correct: {torch.allclose(cuda_d_cb, torch_d_cb, atol=atol, rtol=rtol, equal_nan=False)}")
	print(f"cuda d_cb absolute error: {abs_error:.5f}")
	print(f"cuda d_cb relative error: {rel_error:.5f}")
	print(f"cuda d_cb percent error: {rel_error*100:.5f}%\n")
	...
	# print((torch_d_cb/cuda_d_cb).mean(dim=1))
	# print(wavenumbers)

def wf_embedding_torch(coordsA, coordsB, cb_magnitudes, wavenumbers, mask=None):
	...
	# cb_labels : Z x N
	# cb_magnitudes : K x AA
	# coords B should end up being Z x N x K x 3
	
	coordsB = coordsB[:, :, None, :] * cb_magnitudes[None, None, :, None] # Z x N x K x 3
